...
func main() {
	...
	// Create handler for processing websocket SpotBalanceUpdateEvent
	spotHandler := func(event stream.SpotBalanceUpdateEvent) {
		fmt.Printf("%#v\n", event)
	}
	...
	streamService.Query(stream.NewSpotBalanceCommand([]string{"BTC", "USDT"}), func(command stream.Command, response []byte) {
		var result map[string]stream.AssetBalance
