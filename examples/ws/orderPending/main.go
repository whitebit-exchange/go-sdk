...
func main() {
	...
	// Create handler for processing websocket PendingOrdersUpdateEvent
	pendingOrdersHandler := func(event stream.PendingOrdersUpdateEvent) {
		fmt.Printf("%#v\n", event)
	}
	...
	//Send PendingOrders query with handler for result processing
	streamService.Query(stream.NewPendingOrdersCommand("BTC_USDT", 0, 10), func(command stream.Command, response []byte) {
		var result stream.Orders
