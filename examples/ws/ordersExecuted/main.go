...
func main() {
	...
	// Create handler for processing websocket OrderExecutedUpdateEvent
	executedOrdersHandler := func(event stream.OrderExecutedUpdateEvent) {
		fmt.Printf("%#v\n", event)
	}
	...
	//Send OrdersExecuted query with handler for result processing
	streamService.Query(stream.NewOrdersExecutedCommand("BTC_USDT", []int64{stream.TypeLimit, stream.TypeMarginMarket}, 1, 0), func(command stream.Command, response []byte) {
		var result stream.OrdersExecuted
