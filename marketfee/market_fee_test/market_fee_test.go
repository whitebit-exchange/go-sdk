...
func TestMarketFeeGetMarketFeeWithWireMock(
    t *testing.T,
) {
    wiremockPort := os.Getenv("WIREMOCK_PORT")
    	if wiremockPort == "" {
    ...
    _, invocationErr :=     client.MarketFee.GetMarketFee(
            context.TODO(),
            request,
            option.WithHTTPHeader(
                http.Header{"X-Test-Id": []string{"TestMarketFeeGetMarketFeeWithWireMock"}},
            ),
        )

    require.NoError(t, invocationErr, "Client method call should succeed")
    VerifyRequestCount(t, "TestMarketFeeGetMarketFeeWithWireMock", "POST", "/api/v4/market/fee", map[string]string{"market": "BTC_USDT"}, 1)
}
